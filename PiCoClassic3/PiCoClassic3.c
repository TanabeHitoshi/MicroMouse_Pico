...
//#include "parameters_PENGIN.h"
//#include "parameters_RED.h"
#include "init.h"
#include "glob_var.h"
#include "run.h"
...
extern void fast_run2(int x, int y);
extern void fast_run3(int x, int y);
...
//			fast_run(0,0);					//スタート地点まで最短走行
		...
		case 5:
			mypos.x = mypos.y = 0;			//座標を初期化
			mypos.dir = north;				//方角を初期化
			search_adachi(GOAL2_X,GOAL2_Y);	//ゴールまで足立法
//			rotate(right,2);				//ゴールしたら180度回転する
//			mypos.dir = (t_direction)((mypos.dir+6) % 4);	//方角を更新
			search_adachi(GOAL_X,GOAL_Y);	//ゴールまで足立法
			goal_appeal();					//ゴールしたことをアピール
		...
		case 6:		//最短走行 2
			...
			mypos.dir = north;				//方角を初期化
			accel = 1.5;					//加速度を設定
			fast_run2(GOAL_X,GOAL_Y);		//ゴールまで最短走行
			mypos.dir = (t_direction)((mypos.dir+6) % 4);		//方角を更新
			rotate(right,2);				//ゴールしたら180度回転
			goal_appeal();					//ゴールしたことをアピール
//			fast_run(0,0);					//スタート地点まで最短走行
		...
		case 7:		//最短走行 3
			...
			mypos.dir = north;				//方角を初期化
			accel = 1.5;					//加速度を設定
			fast_run3(GOAL_X,GOAL_Y);		//ゴールまで最短走行
			mypos.dir = (t_direction)((mypos.dir+6) % 4);		//方角を更新
			rotate(right,2);				//ゴールしたら180度回転
			goal_appeal();					//ゴールしたことをアピール
//			fast_run(0,0);					//スタート地点まで最短走行
		...
		case 8:			mypos.x = mypos.y = 0;			//座標を初期化
			mypos.dir = north;				//方角を初期化
			search_adachi(0,15);				//v左上
			search_adachi(15,15);				//v右上
			search_adachi(15,0);				//v左上
//			rotate(right,2);				//ゴールしたら180度回転する
//			mypos.dir = (t_direction)((mypos.dir+6) % 4);	//方角を更新
			search_adachi(GOAL_X,GOAL_Y);	//ゴールまで足立法
//			rotate(right,2);				//ゴールしたら180度回転する
